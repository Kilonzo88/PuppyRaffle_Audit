...
\begin{titlepage}
    ...
    \begin{figure}[h]
        \centering
        \includegraphics[width=0.5\textwidth]{audit_data/logo.pdf} 
    \end{figure}
    \vspace*{2cm}
...
This is a raffle in which participants can enter and win a random puppy. The owner of the protocol can set a feeAddress to take a 20%cut of the `entranceFee` for gas fees, and the rest of the funds will be sent to the winner of the puppy.
...
A comprehensive security audit was conducted on the PuppyRaffle smart contract, a raffle protocol where participants can enter to win a random puppy NFT. The review identified **17 total findings**, including **3 critical high-severity vulnerabilities** that pose significant risk to user funds and protocol integrity.

**Critical findings include:**
- A reentrancy vulnerability in the refund mechanism that allows malicious actors to drain the entire contract balance
- Weak randomness implementation enabling validators to manipulate or predict raffle winners
- Integer overflow issues in fee accounting that result in permanent loss of protocol revenue

The protocol is built on Solidity 0.7.6, an outdated version that lacks built-in overflow protections and modern security features. Multiple functions fail to follow the Checks-Effects-Interactions pattern, and the randomness mechanism relies on predictable on-chain values rather than a verifiable random function.

**The PuppyRaffle contract is not safe for production deployment in its current state.** Immediate remediation of the high-severity findings is required, along with upgrading to Solidity ^0.8.18, implementing proper access controls and reentrancy guards, and replacing the randomness mechanism with Chainlink VRF or similar cryptographically secure solution.

With the recommended fixes implemented, the protocol's core functionality can be preserved while significantly improving its security posture.
